...
- 家計簿連携（kakeibo-bridge API）— 1回の入力から最大10件の取引を候補化し、1件ずつ確認・1件ずつ登録
...
**一括登録ではありません。** 「1取引 = 1確認 = 1 POST」という安全境界はそのままで、変わったのは入口だけです。候補は1件ずつ確認し、1件ずつ登録します。

```text
入力: スーパーで2000円、コンビニで500円、薬局で1200円

  (1 / 3) スーパー  2,000円  → 確認して登録 → POST #1
  (2 / 3) コンビニ    500円  → 確認して登録 → POST #2
  (3 / 3) 薬局      1,200円  → 確認して登録 → POST #3
```
...
| 途中失敗 | 残りの候補を処理せず停止します。登録済みの取引は取り消しません |
...
さらに、次の場合は**推測せず入力全体を拒否**します（部分登録を防ぐため）。
...
確認・登録シーケンスが進行中の間は、**キーボード入力・音声入力とも新しい送信を開始しません**（`Controller.is_busy()`）。完了・スキップ・中止・エラーのいずれでも解除されます。
...
| 項目 | 旧版の記述 | 現行コードの実装 |
|---|---|---|
| `kakeibo_date.py` | 「おととい」「先週の金曜」などを日付に直す | **自然言語の相対日付を解析するコードはありません。** 対応形式は `YYYY/MM/DD`、`YYYY-MM-DD`、`M/D` のみ |
...
| 家計簿・健康記録の登録 | 本アプリの機能として紹介 | **別途ローカルAPIが必要。** 家計簿側は2026-08-18時点で非公開 |
...
実在しない暦日（`2026/02/30`）、範囲外の月（`2026/13/01`）、区切り文字の混在（`2026/08-16`）は日付として扱わず、実行日へフォールバックします。`2026年8月16日` のような和暦区切り形式は未対応です。
...
- 家計簿連携に使用する`kakeibo-bridge`の`8767`は専用Composeで管理し、ホストの`127.0.0.1`だけに公開しています
