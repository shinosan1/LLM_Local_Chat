...
初回暗号化前であれば、ソースの部分バックアップから変更対象を戻し、新規追加された`history_crypto.py`と`tests/test_history_crypto.py`を除くことで旧版ソースへ戻せます。バックアップ名に`v1.4.2`が含まれていても、内容はv1.4.2適用前の部分バックアップです。
