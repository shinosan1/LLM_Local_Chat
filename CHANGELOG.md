...
### ドキュメント
- READMEにv1.4系の主要な精度・停止処理・TTS排他・Biolog検証・履歴管理の改善を追加
- 家計簿APIの`8000`をホスト非公開とする旧記述を、現行Composeの`127.0.0.1`限定公開へ訂正
- `temperature=0.7`と`max_tokens=1024`がコード既定値・配布テンプレート値であること、および既存設定とVRAM制御が優先されることを明記
